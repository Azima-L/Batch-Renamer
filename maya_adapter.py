...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QListWidget, QListWidgetItem, QRadioButton, QPushButton, QFileDialog
...
def load_from_scene():
    ...
    return [obj for obj in all_transforms if obj not in DEFAULT_NODES]

def maya_renamer(old_name, new_name):
    if cmds.objExists(old_name):
        cmds.rename(old_name, new_name)
        print(f'[INFO] Renamed "{old_name}" → "{new_name}"')
    else:
        print(f'[WARN] "{old_name}" not found in scene — skipped')

class MyMayaWindow(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def refresh_scene(self):
        assets = load_from_scene()
        self.input_list.clear()
        for asset in assets:
            self.input_list.addItem(asset)
        print(f'[INFO] Added "{asset}" to the input name list.')
    ...
    def apply_renames(self):
        if not self.preview_results:
            print('[WARN] Nothing to apply. Run Preview first.')
            return
        else:
            for old_name, new_name in self.preview_results:
                maya_renamer(old_name, new_name)
            print(f'[INFO] Applied {len(self.preview_results)} renames to scene.')
